...
if [[ $# -ne 3 ]]; then
    echo "** Usage: $0 <controller IP> <PUBLIC_INTERFACE_NAME> <OVERLAY_INTERFACE_IP_ADDRESS>"
    exit 1
fi
...
configure_ml2_plugin () {
  ...
  openstack-config --set ${CONF} ml2_type_flat flat_networks public
...
configure_linux_bridge_agent () {
  ...
  openstack-config --set ${CONF} linux_bridge physical_interface_mappings public:${PUBLIC_INTERFACE_NAME}
